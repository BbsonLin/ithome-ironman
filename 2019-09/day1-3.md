...
> * 如果想使用 Apple iOS 風格設計可使用 `CupertinoApp` 來構建，基本上直接使用 `MaterialApp`就有得玩了~
...
在 `lib/main.dart` 裡可以看到程式碼中 `GitmeRebornApp` 的 `home` 屬性裡面我填了 `LoginPage`，這使得剛跑起 App 時就會自動顯示出我寫的登入的畫面 (`lib/pages/login.dart`)。
